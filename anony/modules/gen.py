...
async def _generate(_, cq: types.CallbackQuery):
    await cq.answer()
    await cq.message.reply_text("<b>» Please choose the library for which you want generate string :</b>", reply_markup=buttons.gen_key())
...
async def _gen_session(_, cq: types.CallbackQuery):
    ...
    try:
        txt = "Here is your {0} session\n\n<code>{1}</code>\n\nA session generator bot by <a href='https://t.me/ll_CarelessxCoder_ll'>˹ᴄᴀʀᴇʟᴇss ꭙ ᴄᴏᴅᴇʀ˼ </a>\n☠ <b>Note :</b> Don't share the session with anyone."
        if pyrogram:
            ...
            try:
                await client.join_chat("CarelessxWorld")
            except:
                pass
        else:
            ...
            try:
                await client(JoinChannelRequest("@ll_CarelessxCoder_ll'>"))
            except:
                pass
    except KeyError:
        pass
    try:
        ...
        await cq.message.reply_text(f"Successfully generated your {sgen} string session.\n\nPlease check your saved messages for getting it.\n\nA string is generated by <a href='https://t.me/ll_CarelessxCoder_ll'>˹ᴄᴀʀᴇʟᴇss ꭙ ᴄᴏᴅᴇʀ˼ </a>.", reply_markup=buttons.pm_key(cq.from_user.id))
    except:
        pass
